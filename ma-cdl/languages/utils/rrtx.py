...
class Pdm4arAgent(Agent):
    ...
    def on_episode_init(self, init_obs: InitSimObservations):
        ...
        self.lanelet_network_poly: geom.Polygon = shapely.ops.unary_union(
            polygons)
        self.x_bounds = (
            self.lanelet_network_poly.bounds[0], self.lanelet_network_poly.bounds[2])
        self.y_bounds = (
            self.lanelet_network_poly.bounds[1], self.lanelet_network_poly.bounds[3])
        ...
        if self.LOG_TELEM:
            plot_things([geom.Point(self.furthest_point[0], self.furthest_point[1])],
                        self.RRT.static_obs+self.RRT.X_obs_d +
                        [self.RRT.lanelet_ext],
                        self.RRT.all_nodes,
    ...
    def get_commands(self, sim_obs: SimObservations) -> VehicleCommands:
        ...
        # steering angle
        curr_delta = sim_obs.players[self.name].state.delta
        curr_vx = sim_obs.players[self.name].state.vx

        # Polygon representing current car
        curr_obs = sim_obs.players[self.name].occupancy
        ...
        # X pos of front axle
        fa_x: float = curr_x + np.cos(curr_theta)*self.sg.lf
        # Y pos of front axle
        fa_y: float = curr_y + np.sin(curr_theta)*self.sg.lf

        if self.PRINT_PROFILING:
            self.prof.enable()
        # State update for RRT every few iterations
        if self.time_step % 1 == 0:
            self.RRT.step(v_curr=(curr_x, curr_y, curr_theta),
                          new_obs=other_obs)
        ...
        if self.traj is None:
            ...
            self.traj = np.array(
                [(curr_x, curr_y), (self.RRT.v_bot.p_T_out.x, self.RRT.v_bot.p_T_out.y)])
            ...
            cross_err, ang, cross_pt, closest_parent = get_crosstrack_err(
                fa_x, fa_y, self.traj)
        ...
        ddelta = self.kp_delta*(des_delta-curr_delta) + self.kd_delta * \
            ((des_delta-curr_delta)-self.prev_delta_err)
        ...
        acc = self.kp_acc*(des_speed-curr_vx) + self.kd_acc * \
            ((des_speed-curr_vx)-self.prev_vx_err)
        ...
        if self.PRINT_HEARTBEAT:
            ...
            print(
                f"Robot at ({curr_x:.2f}, {curr_y:.2f}), {np.degrees(curr_theta):.2f}, {curr_vx:.2f}ms | Goal: {self.furthest_point[0]:.1f}, {self.furthest_point[1]:.1f}")
            # print(f"cross_err: {cross_err:.2f}, cross_pt: ({cross_pt.x:.2f}, {cross_pt.y:.2f}), front axle: {fa_x:.2f}, {fa_y:.2f}")
            print(
                f"Current speed: {curr_vx:.2f}, Desired speed: {des_speed:.2f}, lookahead: {lookahead_cost:.2f}, deviation: {deviation_cost:.2f}, clearance_cost: {clearance_cost:.2f}")
        ...
        if self.LOG_TELEM:
            ...
            if self.time_step % self.PLOT_INTERVAL == 0 and self.time_step > 0:
                if self.PRINT_PROFILING:
                    stats = pstats.Stats(
                        self.prof).strip_dirs().sort_stats("cumtime")
                ...
                plot_things(
                    ...
                    self.RRT.static_obs+self.RRT.X_obs_d +
                    [self.RRT.lanelet_ext]+other_players_circ,
                    self.RRT.all_nodes,
                    # self.x_bounds, self.y_bounds,     # Uncomment to plot globally
                    (plot_ctr_x-25.0, plot_ctr_x+25.0), (plot_ctr_y - \
                                                         25.0, plot_ctr_y+25.0),     # Plot centered on car
...
def get_crosstrack_err(fa_x: float, fa_y: float, cs: NDArray) -> Tuple[float, geom.Point, float]:
    ...
    heading = np.arctan2(
        closest_parent[1]-cross_pt.y, closest_parent[0]-cross_pt.x)
...
def get_predicted_position(state: X, occupancy: geom.Polygon, L: float, Lr: float, dt: float) -> geom.Polygon:
    ...
    shifted = affi.rotate(occupancy, d_theta,
                          origin='center', use_radians=True)
...
class RRT_X:
    def __init__(self, v_goal: RRT_Node, static_obs: List[geom.Polygon],
        ...
        self.Vq: Dict[KDState, RRT_Node] = {
            self.goal_node.kd(): self.goal_node}
        ...
        self.pos_polygon: geom.Polygon = geom.Point(
            v_goal.x, v_goal.y).buffer(0.01)  # dummy init
    ...
    def get_trajectory(self) -> NDArray:
        ...
        while p_it is not None:
            ...
            # infinite loop
            assert p_it.p_T_out is not p, f"The parent of p_it:{p_it} is {p}."

            if p_it.is_in_collision:
                print(
                    f"{p_it} collides with an obstacle. This trajectory is invalid.")
                return None

            # There must be a loop somewhere if this happens
            if its > 100:
                print(
                    "Took > 100 iterations to find traj. We are likely to have loops in our graph.")
                return None
    ...
    def _update_obstacles(self, new_obs_l: List[geom.Polygon]):
        ...
        assert len(
            self.nodes_in_collision) == 0, f"Not all collisions flags were cleared. {self.nodes_in_collision}"
        ...
        assert len(self.X_obs_d) == len(
            new_obs_l), f"Not all elems of new_obs_l (len {len(new_obs_l)}) were copied to X_obs_d (len {len(self.X_obs_d)})"
    ...
    def _remove_obstacle(self, obs: geom.Polygon):
        ...
        E_o, c_l = self._get_all_colliding_edges(
            obs, coll_rad=self.EDGE_COLL_RAD)
    ...
    def _update_robot(self, v_curr: Tuple[float, float, float]):
        ...
        while True:
            ...
            if self._extend(self.v_bot):
                ...
                print(
                    f"It {self.curr_it} | No nodes near v_bot {self.v_bot} yet. Sampling more pts.")
                self.step()
    ...
    def _static_collision(self, v: RRT_Node) -> bool:
        ...
        curr_poly = geom.Point(v.x, v.y).buffer(
            (self.lf + self.OBST_CLEARANCE))
    ...
    def _extend(self, v: RRT_Node) -> bool:
        ...
        if v.p_T_out is None:
            print(
                f"_extend(): No feasible way of getting from {v} to other nodes")
            return False
        ...
        if v.kd() not in self.Vq.keys():
            ...
            self.Vq[v.kd()] = v
            print(
                f"It {self.curr_it:04d}, #Nodes: {self.num_nodes:04d} || Inserting {v.kd()} into Vq. r: {self.r:.2f}")
        else:
            print(
                f"It {self.curr_it:04d}, #Nodes: {self.num_nodes:04d} || {v.kd()} is already in self.Vq. Updating its key.")
...
def plot_things(things: List[geom.base.BaseGeometry], obstacles: List[geom.base.BaseGeometry],
    ...
    for el in points:
        ...
        plt.plot(node[0], node[1], marker_color+'x',
                 markersize=msize, markeredgewidth=mw)
